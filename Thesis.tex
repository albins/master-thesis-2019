...
references to writable memory, Rust is able to make greater guarantees of
...
Due to limitations in its formulation, the current borrow checker, \textit{non-linear
lifetimes} (NLL), rejects code such as the one in
...
flow-sensitive, that is reason at the power of each individual program
...
\colorbox{green}{green} and \colorbox{red}{red} boxes illustrate where the
...
\caption[The Rules of the Borrow Check]{The Rules of the borrow check, with
  \colorbox{green}{positive} (free from errors) and \colorbox{red}{negative}
