...
Summing up the ongoing work of \citeauthor{weiss_oxide:_2019} on formalising the
reference ownership rules of Rust~\cite{weiss_oxide:_2019}. At the heart of the
type system lies the flow-sensitive typing judgement seen in Rule~\fixme{add fig
  1 of Oxide here.}. \fixme{explain the rules here}.

\fixme{describe the different $\omega$-derivations.}

\fixme{describe the branching rules}

\fixme{describe the sequencing rules}
...
respected. That means, for each point $p$, with environment $\Gamma_p$:

\begin{itemize}
\item moving or unique-borrowing a place $\pi$ implies that for all every loan
  $l$ of $\pi$ in $\Gamma_p l \:\text{Killed}\: p$.
\item shared-borrowing a place $\pi$ creates a new loan $l$ which is in the
  provenance variable of the LHS. \fixme{How do I show that this invalidates any
  shared borrows of $\pi$???}.
\item assigning to a place $\pi$:
  \begin{itemize}
  \item kills the loan associated with $\pi$ from $p$ and onward
  \item enforces a subset relationship between LHS' region and RHS' region
    \fixme{how does this interact with overwriting?!}
  \end{itemize}
\end{itemize}
...
\section{A Field Study of Polonius Input Data}\label{sec:field-study-borrow}
...
number of stars on GitHub, for analysis.~\footnote{Source code for the analysis
  as well as listings of the repositories are available \fixme{on the moon}.} Of
the initially selected repositories only about 1 000 were from other sources
than GitHub. Only crates that compiled under recent versions of Rust nightly
builds with non-linear lifetimes enabled were kept. The source code of the
packages was then translated to Polonius input files for a total of
\fixme{186GB}~of tab-separated tuples, which I used to measure Polonius
solve-time performance as well as for finding common patterns in the input data.
Only complete data sets were considered; a repository with more than one target
where at least one target did not compile was discarded, as was any repository
with more than 8GB~of input data for any relation, or where fact generation took
longer than 15~minutes due to RAM- and time constraints. After this selection
process, \fixme{10 000} repositories remained for the final study.

% what do we want to know?
% what are our proxy variables?
% on what type of machine was the analysis performed?
