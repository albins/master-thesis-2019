...
example, \emph{does} pass the borrow check, as the loans do not overlap:
\begin{minted}{rust}
struct Point(u32, u32);
...
analysed at this granularity, but are considered one object. In Rust compiler
parlance, we say that a path points to a \emph{place}, corresponding literally
to the place in memory holding its data.
...
\InRust{x.y} would have the prefixes \InRust{x} and \InRust{x.y}. This is useful
when reasoning 
...
For this reason, NLL rejects certain desirable patterns of Rust, such as the
code in Listing~\ref{lst:issue-51132}, that Polonius is designed to accept.
Leaving out the flow-sensitive analysis of NLL was motivated by performance
reasons and is one of the motivations for using the Datalog implementation, in
addition to the intention of having a clearer formulation in a data-oriented
language.
...
\item[\InDatalog{child(M1, M2)}] when the move path $M_1$ is the child of $M_2$,
  ...
  child of \InRust{x.y} and \InRust{x}.
...
in this example, only emit \InDatalog{moved_out_at(x, p)} as a starting fact.
Finding the transitive closure given by projecting the move across all of the
path's children would happen in Polonius.
...
if something {
  x = &z; // creates {L0} in 'x constraint
          //   at this point, we have `'x: 'y` and `{L0} in `'x`,
          //   so we also have `{L0} in 'y`
  drop(x);
}
