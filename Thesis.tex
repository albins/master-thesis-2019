...
\begin{table}[h]
  \begin{tabular}{@{}l l m{5.5cm}}
    ...
    \InDatalog{path_init_lub_exit(Path, Point)} & I & \InDatalog{Path} is initialized without subsequent deinitialisation on one or more branches reaching this CFG node (lowest upper bound on set membership).\\
    \InDatalog{var_init_glb_entry(Var, Point)} & I & This variable is initialised on all branches reaching this CFG node (greatest lower bound on membership). \\
    \InDatalog{path_moved_lub_exit(Path, Point)} & I & \InDatalog{Path} has been moved without subsequent reinitialisation on at least one branch reaching this CFG node. \\
    \InDatalog{var_init_lub_exit(V, Point)} & O & This variable is initialised along at least one branch reaching this node at the time of its exit. \\
...
\begin{sourcecode}
  \captionof{listing}{A move error is a path access to any path that is not
    provably initialised.}\label{lst:move-error}
\begin{minted}{prolog}
...
\begin{sourcecode}
  \captionof{listing}{A path is provably initialised if may be initialised and
    may not have been moved.}\label{lst:path-definitely-initialised}
\begin{minted}{prolog}
path_definitely_initialized_at(Path, Point) :-
   path_maybe_initialized_on_exit(Path, Point),
