...
The other motivation for Polonius is to more clearly capture the semantics of the
...
\goodexample{green} and \badexample{red} boxes illustrate where the borrow check
would occur without or with an error, respectively. Many of these examples are
borrowed from \citeauthor*{weiss_oxide:_2019}~\cite{weiss_oxide:_2019}.

{ \renewcommand{\arraystretch}{2.0}
\begin{table}[!htbp]
\begin{tabular}{p{0.18\textwidth} p{0.41\textwidth} p{0.41\textwidth}}
  Rule & Positive Example & Negative Example \\ \toprule
...
\caption[The Rules of the Borrow Check]{The Rules of the borrow check, with
  ...
  variable. Green highlights show accepted uses and red ones show failed ones.}\label{tab:borrow-check}
\end{table}%
}
...
Named provenances, such as \InRust{'lifetime} above, are referred to as
``provenance variables''.~\footnote{In the literature, the terms
  ``region''~\cite{matsakis_alias-based_2018}, ``(named)
...
\begin{tabular}{@{}l l@{} l@{} @{}l@{}}
  ...
  \InDatalog{invalidates(P, L)}\notmine{} &
                                  ...
                                  \end{tabular}
                      & \InRust{(M(bb0[1]), &_1)} & Bck  \\
  \InDatalog{var_used(V, P)} & \InRust{bb0[0]}: \InRust{x + 1} & \InRust{(x, M(bb0[0]))} & Lvs  \\
  \InDatalog{var_defined(V, P)} & \InRust{bb0[0]}: \InRust{x = 7} & \InRust{(x, M(bb0[0]))} & Lvs  \\
  \InDatalog{var_drp_usd(V, P)} & \InRust{bb0[0]}: \InRust{drop(x)} & \InRust{(x, M(bb0[0]))} & Lvs  \\
  \InDatalog{var_uses_rgn(V, R)} & \InRust{let x: &'x i32;} & \InRust{(x, 'x)} & Lvs  \\
  \InDatalog{var_drps_rgn(V, R)} &
                                  \begin{tabular}[t]{@{}l@{}}
                                    \InRust{struct Wr<'p> { p: &'p i32 }} \\
                                    \InRust{impl<'p> Drop for Wr<'p> {...}} \\
                                    \InRust{let x: Wr = ...} \\
                                    \InRust{drop(x)}
                                  \end{tabular}
                      & \InRust{(x, 'p)}& Lvs  \\
...
By reducing the expressive power of the programming language, Datalog enables a
greater degree of automated optimisations of programs, including transferring
the choice of data structures to the compiler. This means that compiler
optimisations can massively improve the performance of user code, and is one of
the reasons why \citeauthor{smaragdakis2019next} mentioned Datalog as an example
of a next-paradigm programming language during the Onward! track of the 2019
SPLASH conference~\cite{smaragdakis2019next}.

\section{Datalog: a Natural Choice for Polonius}\label{sec:why-datalog}

Being convenient for expressing computations of transitive closures, Datalog has
a long history of use for static analysis. A notable recent example being
the Souffl{\'e} system, which synthesises performant C++ code from the Datalog
specifications to show promising performance for analysis of large programs
comparable to hand-crafted implementations in C++~\cite{scholz_fast_2016}.
Souffl{\'e} in particular was an inspiration for Polonius, and with the current
ongoing work on translating Souffl{\'e}-style Datalog into the more cumbersome
Datafrog EDSL format (discussed in the following section) Polonius can almost be
...
\begin{sourcecode}
  ...
  Datafrog.]{\InDatalog{ancestor(X, Y)} in Datafrog. Note the \InRust{map()}
    ...
    reversed target order. This example is used in Polonius to derive children
    of move paths, discussed in
...
Moving on with a more complex example, the Datafrog for
...
transitive closure backwards across the CFG\@. A variable is use-live where it
is being used, and stays use-live (backwards) until it is defined. Specifically,
...
illustration of this is in graph form, as seen in
    ...
    \begin{tabular}{@{}l l}
      $\top$ & \begin{tabular}[t]{@{}l  l@{}}
                   ...
                   \begin{tabular}[t]{@{}l}
                     ...
                     \InRust{var_maybe_init_on_exit(x, Mid(bb0[0]))}\\
                     \InRust{cfg_edge(Mid(bb0[0]), Start(bb0[1]))}\\
                   \end{tabular} &%
                                   \InRust{var_drop_live(x, Start(bb0[1]))} \\ \toprule
...
of the borrow checker. A given provenance variable~$R$ is live at some node~$p$
...
{ \renewcommand{\arraystretch}{1.5}
    \begin{tabular}{@{}l l}
      $\top$ & \begin{tabular}[t]{@{}l  l@{}}
                   ...
                   \end{tabular} &%
                                   \InRust{region_live_at('nested, Mid(bb0[1]))}\\ \toprule
                   \begin{tabular}[t]{@{}l}
...
\begin{table}[!htbp]
\begin{tabular}{@{}l l l l}
  Statement & Use-live & Drop-live & Provenance(s) live \\ \toprule
  \InRust{let mut x = 13;} \\
...
\caption[Liveness Example]{An example of a drop-live \InRust{struct} causing an
  ...
  mutable.}\label{tab:liveness-example}
\end{table}%
 }
...
\begin{tabular}{@{}l l l l}
  ...
  \InRust{move x.0}        & \InRust{x.1}, \InRust{x}               & \InRust{x.0} & \InRust{x.0} \\ % a move
  \InRust{x.1 + 7}         & \InRust{x.1}, \InRust{x}               & \InRust{x.0} & \InRust{x.1} \\ % a legal access to partially deinitialised
  \InRust{x.0 + 3}         & \InRust{x.1}, \InRust{x}               & \InRust{x.0} & \InRust{x.0} \\ % illegal access to partially deinitialised
  \InRust{x.0 = 4}         & \InRust{x.1}, \InRust{x.0}, \InRust{x} &   \\ % re-initialisation
  \InRust{if random() {move x.0}}  & \InRust{x.1}, \InRust{x.0}, \InRust{x} & \InRust{x.0}        \\ % imprecision
  \InRust{f(x)}  & \InRust{x.1}, \InRust{x.0}, \InRust{x} & \InRust{x.0} & \InRust{x}, \InRust{x.0}, \InRust{x.1}       \\ % access to paths
\end{tabular}
...
\begin{sourcecode}
  \captionof{listing}{Calculating transitive moves: a move, initialisation, or
    ...
    the prefix' children. Identical rules for for \InDatalog{path_assigned_at(P,
      _)}, \InDatalog{path_begins_with_var(P, V)}, and
...
introduced by the join to \InDatalog{cfg_edge(From, To)}; it is enough for one
connecting edge to \InDatalog{To} to have \InRust{x.f} initialised for it to be
initialised at \InDatalog{To}. To handle function arguments and local variables,
we simulate an assignment for each argument and a move for each local at the
start of the function.

Finally, we relate the initialisation of root paths back to their variables
through \InDatalog{path_begins_with_var(P, V)}. This means that we track
variables that are partially initialised; an over-approximation needed because a
deallocation of a variable holding a partially initialised \InRust{struct} would
only be guaranteed to be a no-op if the entire variable was moved at the time of
the deallocation. The full listing for the code can be found in
...
{ \renewcommand{\arraystretch}{1.5}
    \begin{tabular}{@{}l l}
      $\top$ & \begin{tabular}[t]{@{}l  l@{}}
                   Input & Conclusion \\ \toprule
                   \begin{tabular}[t]{@{}l}
                     \InRust{path_assigned_at(x, Mid(bb0[0]))}\\
                     \InRust{cfg_edge(Mid(bb0[0]), Start(bb0[1]))}\\
                     \InRust{path_begins_with_var(x, x)}\\
                   \end{tabular} &%

                   \begin{tabular}[t]{@{}l}
                     \InRust{var_m_i_e(x, Mid(bb0[0]))} \\
                     \InRust{var_m_i_e(x, Start(bb0[1]))}
                   \end{tabular}
      ...
      $\bot$ & \begin{tabular}[t]{@{}l  l@{}}
                   Input & Conclusion \\ \toprule
                   \begin{tabular}[t]{@{}l}
                     \InRust{path_ma_in_on_ex(x, Start(bb0[0]))}\\
                     \InRust{path_moved_at(x, Mid(bb0[0]))}\\
                     \InRust{cfg_edge(Start(bb0[0]), Mid(bb0[0]))}\\
                     \InRust{path_begins_with_var(x, x)}\\
                   \end{tabular} &%
...
\begin{sourcecode}
  \captionof{listing}{A move error is a path access to any path that may be
    deinitialised. We use a join with the CFG to move the error one node ahead
    from the last provably initialised node. This is because if a path is
    initialised on \textit{exit} from some statement, it is still initialised on
    \textit{entry} to the next one, which would correspond to the node where
    the evaluation (but not the effect) of the statement happens. Without this
    ``rolling up'', we would have a move error on every statement that moves a
    path, as that statement also accesses the path it moves. With these rules,
    the path would be accessed at the starting-poing of the move statement, and
    moved at mid-point, thus avoiding generating an error where they
    overlap.}\label{lst:move-error}
\begin{minted}{prolog}
...
\begin{sourcecode}
  \captionof{listing}{A \InDatalog{Path} may have been moved at a
    \InDatalog{Node} if it was moved on the way there without being
...
\begin{sourcecode}
  \captionof{listing}{Subset relations hold at the node where they are
...
that each provenance variable~$R$ contains the loan~$L$ that created it at node
the node~$P$ where the borrow occurred.

\begin{sourcecode}
  \captionof{listing}{A provenance variable\InDatalog{P} trivially contains
    (\InDatalog{requires(P, L, _)}) the loan~\InDatalog{L} which introduced
...
Additionally, the \InDatalog{requires(R, L, _)}~relation needs to be propagated
together with subset constraints; after all $R_1 \subseteq R_2$ implies that
$R_2$ must contain all of $R_1$'s~loans. This is captured by the rule in
Listing~\ref{lst:requires-subset}.
    ...
    \begin{tabular}{@{}l l}
      ...
      $\bot$ & \begin{tabular}[t]{@{}l  l@{}}
                   ...
                   \begin{tabular}[t]{@{}l}
                     ...
                     \InRust{cfg_edge(Start(bb0[0]), Mid(bb0[0]))}
                   \end{tabular} &%
                                   (nothing; the loan was killed)\\ \toprule
                   \begin{tabular}[t]{@{}l}
                     \InRust{requires('a, &x, Start(bb0[0]))}\\
...
\begin{figure}
  ...
  \caption[Suggested Refactoring for the Polonius Fact Generation in Rust]{A
    ...
    necessary for producing them.}\label{fig:fact-refactor}
\end{figure}
